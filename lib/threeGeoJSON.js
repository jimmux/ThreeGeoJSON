...
function drawThreeGeo(json, radius, shape, options) {
    ...
    //TODO: Hitting the 65,536 vertex limit, so split model up into chunks.
    // Easy for points, lines may want edge points duplicated to avoid gaps.
    const LIMIT_VERTEX = 65535;
    var pointBuffer = [];
...
//    clearArrays();
//}
function drawParticles(particles, options, keepBuffering) {
    // particles is array of {x, y, z} objects
    var particle_geom = new THREE.Geometry();
    particle_geom.vertices.push(new THREE.Vector3(particles.x, particles.y, particles.z));
